...
def search_notes():
    ...
    return jsonify(SearchResultsResponse(items=results).model_dump()), 200
...
def get_version_diff(note_id: str, version_id: str):
    ...
    diff_chunks = note_service.compute_diff(note["body"], version["body"])
    return jsonify(DiffResponse(items=diff_chunks).model_dump()), 200
