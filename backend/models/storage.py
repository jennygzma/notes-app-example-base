...
class LocalStorage:
    ...
    def _init_files(self):
        for file in [self.notes_file, self.planner_items_file, self.inspirations_file, 
                     self.categories_file, self.links_file, self.folders_file, 
                     self.note_folders_file]:
            if not file.exists():
    ...
    def delete_folder(self, folder_id: str) -> bool:
        ...
        if len(filtered) < len(folders):
            ...
            filtered_mappings = [nf for nf in note_folders if nf["folder_id"] != folder_id]
            self._write_json(self.note_folders_file, filtered_mappings)
            return True
        return False
    ...
    def get_folder_notes(self, folder_id: str) -> List[Dict]:
        note_folders = self._read_json(self.note_folders_file)
        note_ids = [nf["note_id"] for nf in note_folders if nf["folder_id"] == folder_id]
        notes = self.get_notes()
        return [n for n in notes if n["id"] in note_ids]
    
    def set_note_folders(self, note_id: str, folder_ids: List[str]) -> List[Dict]:
        note = self.get_note(note_id)
        if not note:
            raise ValueError(f"Note {note_id} not found")
        ...
        note_folders = self._read_json(self.note_folders_file)
        filtered = [nf for nf in note_folders if nf["note_id"] != note_id]
        
        new_mappings = [{"note_id": note_id, "folder_id": fid, "created_at": self._now()} for fid in folder_ids]
        filtered.extend(new_mappings)
        
        self._write_json(self.note_folders_file, filtered)
        return self.get_note_folders(note_id)
